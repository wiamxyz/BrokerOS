...
export function Workspace({view}:{view:string}){
 ...
 const[collapsed,setCollapsed]=useState(false);const expandButton=useRef<HTMLButtonElement>(null);const collapseButton=useRef<HTMLButtonElement>(null);const[actionTarget,setActionTarget]=useState<HTMLDivElement|null>(null);
 ...
 function toggleSidebar(){const next=!collapsed;setCollapsed(next);try{localStorage.setItem('brokeros-sidebar-collapsed',String(next));}catch{}requestAnimationFrame(()=>(next?expandButton:collapseButton).current?.focus());}
 ...
 return <div className={`app-shell ${collapsed?'sidebar-collapsed':''}`}><MobileSidebarSwipe openMobile={mobile} setOpenMobile={setMobile}/><aside className="sidebar" inert={collapsed}>{navigation(false)}</aside><Sheet open={mobile} onOpenChange={setMobile}><SheetContent side="left" className="mobile-nav" overlayClassName="mobile-nav-overlay" showCloseButton={false} onCloseAutoFocus={event=>{if(search)event.preventDefault();}}><SheetHeader className="sr-only"><SheetTitle>Navigation</SheetTitle><SheetDescription>BrokerOS workspace pages</SheetDescription></SheetHeader>{navigation(true)}</SheetContent></Sheet><div className="main-shell"><header className="topbar">
  <div className="header-navigation">
   ...
   <Button variant="ghost" size="icon-sm" aria-label="Open navigation" title="Open navigation" className="mobile-toggle" onClick={()=>setMobile(true)}><PanelLeft/></Button>
  ...
  <div className="topbar-actions"><div ref={setActionTarget}/><Button size="icon" variant="outline" className="notification-button" aria-label="Priority notifications" onClick={()=>setNotifications(true)}><Bell/>{due.length>0&&<span/>}</Button>{!['campaigns','chat','assistant','insights','settings'].includes(view)&&<Button onClick={()=>setForm(view==='deals'?'deal':view==='today'||view==='tasks'?'task':view==='owners'?'owner':'contact')}><Plus/><span className="action-label">{action}</span></Button>}</div>
