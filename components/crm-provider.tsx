...
import { CRMState, seedState, uid, Activity, Contact, Approval, today } from '@/lib/model';
import { Skeleton } from '@/components/ui/skeleton';
const KEY='brokeros-demo-v1';
type Context={data:CRMState;setData:(update:(prev:CRMState)=>CRMState)=>void;notify:(message:string)=>void;log:(text:string,contactId?:string,kind?:Activity['kind'])=>void;saveContact:(contact:Contact)=>void;approve:(approval:Approval)=>void;reset:()=>void;storageError:boolean};
...
export function CrmProvider({children}:{children:React.ReactNode}){
 ...
 return <CrmContext.Provider value={{data,setData,notify,log,saveContact,approve,reset,storageError}}>{children}{toast&&<div role="status" className="toast">✓ {toast}</div>}</CrmContext.Provider>;
}
